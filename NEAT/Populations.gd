...
func  _init( s_id, species_population, genome):
	self.species_id = s_id
	self.species_pop = species_population
	...
	for i in range(self.species_population):
		self.genomes = i.genome.clone()
	for i in range(1, self.species_pop):
...
func create_next_generation(replicate_ids):
		genomes = {}
...
func crossover(random_genome, random_genome_mate):
		if random_genome.fitness > random_genome_mate.fitness:
			fit_genome = random_genome
			unfit_genome = random_genome_mate
		else:
			fit_genome = random_genome_mate
			unfit_genome = random_genome
...
func increment_population(population_change):
	self.species_population += population_change


func decrement_population(population_change):
	self.species_population -= population_change


func get_skewed_random_sample(n, slope=-1.0):
	"""
...
func pretty_print_gen_id(gen_id):
	print ("-----------------------")
